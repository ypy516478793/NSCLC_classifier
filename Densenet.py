...
def Evaluate(sess):
    ...
    test_loss, test_preds = sess.run([cost, preds], feed_dict=test_feed_dict)
    auroc = compute_acc(y_test, test_preds)
    actual_test = np.argmax(y_test, axis=-1)
    predict_test = np.argmax(test_preds, axis=-1)
    precision, recall, f1 = prf_generator(actual_test, predict_test)

    return auroc, precision, recall, f1, test_loss
...
def get_confusion_matrix_test(sess, x_data, y_data):
    test_feed_dict = {
        x: x_data,
        label: y_data,
        # learning_rate: epoch_learning_rate,
...
#     auroc = compute_acc(test_label, test_preds)
...
def compute_acc(all_batch_labels, all_batch_preds):
    a = np.argmax(all_batch_labels, axis=-1) == np.argmax(all_batch_preds, axis=-1)
    return sum(a) / len(a)
...
# h5f_train.close()
#
# h5f_test = h5py.File('/home/cougarnet.uh.edu/pyuan2/Documents/Data/chest256_val_801010_no_normal.h5', 'r')
...
conditions = ['large cell', 'squamous cell carcinoma', 'nos', 'adenocarcinoma']

indices = [i for i, x in enumerate(y_test) if x == "NA"]
x_test = np.delete(x_test, indices, 0)
y_test = [x for i, x in enumerate(y_test) if i not in indices]
indices = [i for i, x in enumerate(y_train) if x == "NA"]
x_train = np.delete(x_train, indices, 0)
y_train = [x for i, x in enumerate(y_train) if i not in indices]
...
# a = perturbation.random_rotation_2d(x_train, 90)
# b = perturbation.add_noise(x_train)
...
id1_zero = std1 == 0
id1_non_zero = std1 != 0
x_train[:, id1_non_zero] = (x_train[:, id1_non_zero] - mean1[id1_non_zero])/std1[id1_non_zero]
x_train[:, id1_zero] = 0
...
id2_zero = std2 == 0
id2_non_zero = std2 != 0
x_test[:, id2_non_zero] = (x_test[:, id2_non_zero] - mean2[id2_non_zero])/std2[id2_non_zero]
x_test[:, id2_zero] = 0
...
with tf.Session() as sess:
    ...
    all_batch_preds = all_batch_labels = np.zeros((0, class_num))
    for epoch in range(1, total_epochs + 1):
        ...
        for step in range(1, iteration + 1):
            ...
            batch_x = perturbation.random_rotation_2d(batch_x, 180)
            # batch_y = perturbation.add_noise(batch_y)
            # else :
            ...
            if step%freq == 0 :
                ...
                auroc = compute_acc(all_batch_labels, all_batch_preds)
                actual_class = np.argmax(all_batch_labels, axis=-1)
                pred_class = np.argmax(all_batch_preds, axis=-1)
                precision, recall, f1 = prf_generator(actual_class, pred_class)

                # if len(precision) < 5 or len(recall) < 5 or len(f1) < 5:
                #     print("")

                if step == (iteration + 1) // freq * freq:
                    print("------------training confusion matrix------------")
                    get_confusion_matrix(actual_class, pred_class)
                ...
                # test_acc, precision, recall, f1, test_loss, test_summary = Evaluate(sess)
                train_summary = tf.Summary(value=[tf.Summary.Value(tag='Loss', simple_value=loss)])
                train_writer.add_summary(train_summary, iteration*(epoch-1) + step)

                with tf.name_scope('Accuracy'):
                    summary_tr = tf.Summary(value=[tf.Summary.Value(tag='Accuracy', simple_value=auroc)])
                ...
                summary_tr = tf.Summary(value=[tf.Summary.Value(tag='Precision/Mean_Precision',
                ...
                # preds_frq_steps = np.zeros((batch_size*freq, class_num))
                index = 0
                print("Iter " + str(step//freq) + " accuracy: {0:.2f} , Precision: {1:.2f}, Recall: {2:.2f}, F1: {3:.2f}".format(auroc,
                                                                                                   np.mean(precision),
                                                                                                   np.mean(recall),
                                                                                                   np.mean(f1)))
                # print("Iter " + str(step//freq) + " accuracy: {} ".format(auroc))
        ...
        test_auroc, test_precision, test_recall, test_f1, test_loss = Evaluate(sess)
        summary_tr = tf.Summary(value=[tf.Summary.Value(tag='Loss', simple_value=test_loss)])
        test_writer.add_summary(summary_tr, iteration * (epoch - 1) + step)
        summary_tr = tf.Summary(value=[tf.Summary.Value(tag='Accuracy', simple_value=test_auroc)])
        test_writer.add_summary(summary_tr, iteration * (epoch - 1) + step)
        # summary_tr = tf.Summary(value=[tf.Summary.Value(tag='Precision', simple_value=test_precision)])
        # test_writer.add_summary(summary_tr, iteration * (epoch - 1) + step)
        # summary_tr = tf.Summary(value=[tf.Summary.Value(tag='Recall', simple_value=test_recall)])
        # test_writer.add_summary(summary_tr, iteration * (epoch - 1) + step)
        # summary_tr = tf.Summary(value=[tf.Summary.Value(tag='F1', simple_value=test_f1)])
        # test_writer.add_summary(summary_tr, iteration * (epoch - 1) + step)
        ...
        # print(line)
        print("Test accuracy: {}, Test Precision: {}, Test Recall: {}, Test F1: {}".format(np.mean(test_auroc), np.mean(test_precision),np.mean(test_recall), np.mean(test_f1)))
        ...
        actual, predicted = get_confusion_matrix_test(sess, x_test, y_test)

    with open("results_with_mask", "a") as file:
        file.write("--------------------------------exp--------------------------------\n")
        file.write("growth_k: {}, nb_block: {}, total_epochs: {}, batch_size: {}, init_lr: {}, epsilon: {},"
                   " dropout_rate: {} \n".format(growth_k, nb_block, total_epochs, batch_size, init_learning_rate,
